...
// Solution: 
 ...
 const shifterWords = ["H", "I", "M", "N", "O", "S", "X", "Z", "W"];

function shifter(s){
  // your code here
  if (s === "") return 0;
  const uniqueWords = [...new Set(s.split(" ")).values()];
  return uniqueWords
    .filter(word => isWordShifter(word))
    .length
}

function isWordShifter(word) {
  return [...word].reduce((acc, cv) => shifterWords.includes(cv) ? acc : false, true)
}